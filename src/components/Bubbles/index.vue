...
export default {
  ...
  data() {
    return {
      opt: {
        color: "rgba(225, 225, 225, 0.5)", // rgba(225, 225, 225, 0.5)
        radius: 15,
        density: 0.2, // 数值越大，气泡越密集（建议 <=1）
        clearOffset: 0.05, // 气泡消失距离[0-1] 数值越大，消失越晚
      },
      requestAniId: null,
    };
  },
  ...
  mounted() {
    ...
    function Circle() {
      ...
      function init() {
        ...
        self.color =
          settings.color === "random" ? that.randomColor() : settings.color;
      }
      this.draw = function () {
        if (self.alpha <= 0) {
          init();
